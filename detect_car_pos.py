...
if __name__ == '__main__':
    ...
    img = image_tools.crop_to_bounding_box(img, img_bb)
    img_whites = image_tools.crop_to_bounding_box(img_whites, img_bb)
    ...
    mask_r_0 = image_tools.extract_color_mask_min(hsv, 0, 20, sat_min=30, sat_max=95, val_min=200, val_max=255)
    mask_r_1 = image_tools.extract_color_mask_min(hsv, 110, 180, sat_min=30, sat_max=95, val_min=200, val_max=255)
    ...
    conts, _ = cv2.findContours(eroded_r_l4, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
    ...
    for c in conts:
        ...
        if hull_cont.ndim == 3:
            hull_cont = hull_cont.reshape(-1, 1, 2)

        perim = cv2.arcLength(hull_cont, True)

        epsilon = 0.02 * perim

        approx = cv2.approxPolyDP(hull_cont, epsilon, True)
        ...
        if len(approx) == 3 and c_area >= CONTOUR_AREA_MIN and c_area <= CONTOUR_AREA_MAX:
            ...
            #cv2.imshow("cnt", img_cont)

            c_bb = image_tools.find_mask_bounding_box(img_cont)
            print("-> ", c_bb.from_pos, c_bb.to_pos, c_bb.get_size())
            c_bb_w, c_bb_h = c_bb.get_size()

            c_bb_center = image_tools.find_local_box_center(c_bb_w, c_bb_h)

            print(c_bb.from_pos)
            print(c_bb.to_pos)
            print(f"W: {c_bb_w}, H: {c_bb_h}, LB_CENTER: {c_bb_center}")

            cv2.circle(
                img,
                (
                    c_bb.from_pos['x'] + c_bb_center[0],
                    c_bb.from_pos['y'] + c_bb_center[1]
                ),
                4,
                (255,0,255),
                -1
